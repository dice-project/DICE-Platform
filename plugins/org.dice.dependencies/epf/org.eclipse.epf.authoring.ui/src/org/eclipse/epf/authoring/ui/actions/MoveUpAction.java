...
public class MoveUpAction extends AbstractMoveAction {
	...
	private static final ImageDescriptor IMG_DESC = AuthoringUIPlugin
			.getDefault().getImageDescriptor("full/etool16/move_up.gif"); //$NON-NLS-1$
	private static final ImageDescriptor DISABLED_IMG_DESC = AuthoringUIPlugin
			.getDefault().getImageDescriptor("full/etool16/move_up_disabled.gif"); //$NON-NLS-1$ 
	...
	protected Command createMoveCommand(Activity act, Object element,
		...
		return new MoveUpCommand(act, element, classes);
	}	
	...
	protected boolean accept(IStructuredSelection selection) {		
		if(super.accept(selection)) {
			...
			if(ip != null) {
				return !ip.isFirstElement(element);
			}
		}
		return false;
	}
